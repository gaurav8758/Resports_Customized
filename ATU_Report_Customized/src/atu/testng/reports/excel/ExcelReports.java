...
public class ExcelReports
{
  ...
  {
    byte[] arrayOfByte = ExcelChart.writeChartToExcel(paramList1.size(), paramList2.size(), paramList3.size());
  ...
  private static int writeSummaryData(Workbook paramWorkbook, Sheet paramSheet, List<ITestResult> paramList, int paramInt)
  {
    ...
    while (localIterator.hasNext())
    {
      ...
      localCell.setCellValue(CurrentRunPageWriter.getBrowserWithVerion(localITestResult));
      setHyperLink(localITestResult, paramWorkbook, localCell);
  ...
  private static void testStepDescription(Workbook paramWorkbook, Sheet paramSheet, List<ITestResult> paramList)
  {
    ...
    while (localIterator.hasNext())
    {
      ...
      int j = paramSheet.addMergedRegion(new CellRangeAddress(i, i, 0, 7));
      localCell = paramSheet.getRow(i).getCell(j - 1);
      //localCell.setCellStyle(ExcelStyler.setColor(paramWorkbook, ExcelStyler.FOOTER_BG_COLOR, ExcelStyler.FONT_COLOR));
    }
  }
